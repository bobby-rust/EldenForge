...
export default function AI(): JSX.Element {
    ...
    const [showDescription, setShowDescription] =
        React.useState<boolean>(false); // TODO: animate this
    ...
    const [build, setBuild] = React.useState<AIBuildType | null>(
        window.location.pathname === "/ai" ||
            window.location.pathname === "/ai/"
            ? null
            : generator.parseAIBuildFromUrl(
                decodeURIComponent(
                    "?" + buildUrl.toString().replaceAll("+", " "),
                ),
            ),
    );
    ...
    const showHint = () => {
        if (
            localStorage.getItem("acknowledged-hint") === "true" ||
            hintShowed
        ) {
            return;
        }
    ...
    const navigateTo = (path: string) => {
        generator._buildGenerationConfig = structuredClone(
    ...
    return (
        <div className="flex flex-col justify-center items-center xl:px-14 py-8">
            ...
            {!(window.location.search === "") && (
                <div className="flex flex-col items-center 2xl:flex-row justify-evenly w-full mb-10">
                    <div className="flex justify-center items-center w-64 sm:w-80 2xl:w-1/3">
                        <div>
                            <h3>Select a build type (optional)</h3>
                            <select
                            ...
                            >
                                ...
                                <option value="Magic Damage">
                                    Magic Damage
                                </option>
                                <option value="Holy Damage">Holy Damage</option>
                                <option value="Fire Damage">Fire Damage</option>
                                <option value="Lightning Damage">
                                    Lightning Damage
                                </option>
                            </select>
                        </div>
                                                            ...
                                                            {![
                                                                ...
                                                                "items",
                                                                    ...
                                                                    <>
                                                                        ...
                                                                        <td className="p-4 align-middle text-right py-2 px-3">
                                                                            {
                                                                                build[
                                                                                key
                                                                                ]
            ...
            {build && (
                <div
                    className={`max-w-[85vw] 2xl:max-w-[60vw] mb-20 ${showDescription ? "" : "hidden"}`}
                >
                    <div className="w-full bg-gray-100 text-center p-2 relative">
                        <h1 className="font-bold text-slate-800 sm:text-2xl self-start px-8">
                            {build.name}
                        </h1>
                        <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 2lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 2.5xl:grid-cols-6 3xl:grid-cols-7 4xl:grid-cols-8 gap-4 auto-cols-auto">
                {build &&
                    [...build.items.keys()].map(
                        (c: UIItemCategory, i: number) => (
