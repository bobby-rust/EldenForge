...
export default function ItemColumn(props: any) {
	...
	if (props.itemType === "ashes") {
		const ashes = props.build;
		console.log(ashes);
	}
	let name = props.itemType.charAt(0).toUpperCase() + props.itemType.slice(1);
	return (
		<>
			{props.itemType !== "armor" && (
				<Box sx={CONTAINER_THEME}>
					...
					{props.build.map((item: any, i: number) => {
						return <Item key={i} item={item} itemType={props.itemType} buildDispatch={props.buildDispatch} />;
					})}
					{/* <NewItem />
                    <NewItem />
                    <NewItem />
			...
			{props.itemType === "armor" && (
				<Box sx={{ ...CONTAINER_THEME }}>
					<Typography sx={{ fontFamily: "Cormorant Garamond", height: "40px" }}>Armor</Typography>
					{props.build.map((item: any, i: number) => {
						let armorType;
						switch (i) {
							case 0:
